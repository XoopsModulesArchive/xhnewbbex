...
$Id: functions.php,v 1.2 2006/03/28 03:58:44 mikhail Exp $
 ***************************************************************************/
/***************************************************************************
...
function private_forums_list_cant_access($uid, $prefix)
{
    ...
    if (1 == $xoopsModuleConfig['confidentiality']) {
        return '';
    }
    ...
    if (isset($Tbllist) && array_key_exists($uid, $Tbllist)) {
        ...
        while (false !== ($arr = $xoopsDB->fetchArray($result))) {
            ...
            if (0 == $arr2['cpt'] && !is_moderator($arr['forum_id'], $uid)) {
                $cpt++;
                ...
                $flist .= $and . $prefix . 'forum_id<>' . $arr['forum_id'];
            }
        }
...
';
    ...
    if ($result = $xoopsDB->query($sql)) {
        ...
        $myrow['cat_title'] = htmlspecialchars($myrow['cat_title'], ENT_QUOTES | ENT_HTML5);
        ...
        do {
            ...
            if ($res = $xoopsDB->query($sub_sql)) {
                if ($row = $xoopsDB->fetchArray($res)) {
                    do {
                        $name = htmlspecialchars($row['forum_name'], ENT_QUOTES | ENT_HTML5);
...
function xhnewbb_get_topics_viewed()
{
    if (empty($_COOKIE['xhnewbb_topics_viewed'])) {
        return [];
    }

    $topics_tmp = explode(',', $_COOKIE['xhnewbb_topics_viewed']);
    ...
    foreach ($topics_tmp as $tmp) {
        ...
        $id = empty($idmin[0]) ? 0 : (int)$idmin[0];

        $min = empty($idmin[1]) ? 0 : (int)$idmin[1];

        $topics[$id] = $min * 60;
    }

    //$topics = !empty($_COOKIE['xhnewbb_topic_lastread']) ? unserialize($_COOKIE['xhnewbb_topic_lastread']) : array();

    return $topics;
}
function xhnewbb_add_topics_viewed($topicsViewed, $topicId, $timeViewed, $cookiePath, $cookieDomain, $cookieSecure)
{
    ...
    foreach (array_keys($topicsViewed) as $id) {
        $tmp[] = (int)$id . '|' . (int)ceil($topicsViewed[$id] / 60);
    ...
    setcookie('xhnewbb_topics_viewed', implode(',', $tmp), time() + 365 * 24 * 3600, $cookiePath, $cookieDomain, $cookieSecure);

    //$topicsViewed[$topicId] = time();
//setcookie('xhnewbb_topic_lastread', serialize($topicsViewed), time()+365*24*3600, $cookiePath, $cookieDomain, $cookieSecure);
}
