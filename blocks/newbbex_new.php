<?php
// $Id: xhnewbbex_new.php,v 1.3 2006/03/28 03:58:42 mikhail Exp $
// ------------------------------------------------------------------------ //
...
function b_xhnewbbex_new_show($options)
{
    global $xoopsUser;
    ...
    $query = 'SELECT t.topic_id, t.topic_title, t.topic_last_post_id, t.topic_time, t.topic_views, t.topic_replies, t.forum_id, f.forum_name, f.posts_per_page, p.uid FROM '
             . $db->prefix('bbex_topics')
             . ' t, '
             . $db->prefix('bbex_forums')
             . ' f, '
             . $db->prefix('bbex_posts')
             . ' p WHERE (f.forum_id=t.forum_id) AND (p.post_id=t.topic_last_post_id) '
             . $where1
             . ' '
             . $where2
             . ' ORDER BY '
    ...
    while (false !== ($arr = $db->fetchArray($result))) {
        ...
        if (1 == $options[3]) {
            ...
            $topiclink = XOOPS_URL . '/modules/xhnewbbex/viewtopic.php?topic_id=' . $arr['topic_id'] . '&amp;forum=' . $arr['forum_id'] . '&amp;jump=1';
            ...
            $topic['title'] = htmlspecialchars($arr['topic_title'], ENT_QUOTES | ENT_HTML5) . '</a>' . $pagination;
        }
...
function b_xhnewbbex_show_last_messages($options)
{
    ...
    if (mb_strlen(trim($forumslist)) > 0) {
        $forumslist = ' AND (' . $forumslist . ') ';
    }

    $query = 'SELECT t.topic_id, t.topic_title, t.topic_last_post_id, t.topic_time, t.topic_views, t.topic_replies, t.forum_id, f.forum_name, f.forum_type, f.posts_per_page, p.uid FROM '
             . $db->prefix('bbex_topics')
             . ' t, '
             . $db->prefix('bbex_forums')
             . ' f, '
             . $db->prefix('bbex_posts')
             . ' p WHERE (f.forum_id=t.forum_id) AND (p.post_id=t.topic_last_post_id) AND (t.topic_time>$lastvisit) $forumslist ORDER BY t.topic_time DESC';
...
function b_xhnewbbex_new_all_show($options)
{
    global $xoopsUser;
...
function b_xhnewbbex_show_monthly_forums_stat()
{
    global $xoopsUser;
    ...
    while (false !== ($arr = $db->fetchArray($result))) {
        ...
        $forum['forum_name'] = htmlspecialchars($arr['forum_name'], ENT_QUOTES | ENT_HTML5);

        $forum['forum_desc'] = htmlspecialchars($arr['forum_desc'], ENT_QUOTES | ENT_HTML5);

        $forum['forum_cat_id'] = $arr['cat_id'];

        $forum['forum_cat_title'] = htmlspecialchars($TblCateg[$arr['cat_id']], ENT_QUOTES | ENT_HTML5);
...
function b_xhnewbbex_show_forums_stat($options)
{
    global $xoopsUser;
...
function b_xhnewbbex_new_private_show($options)
{
    global $xoopsUser;
    ...
    $query = 'SELECT t.topic_id, t.topic_title, t.topic_last_post_id, t.topic_time, t.topic_views, t.topic_replies, t.forum_id, f.forum_name, f.posts_per_page, p.uid FROM '
             ...
             . ' f, '
             . $db->prefix('bbex_posts')
             . ' p WHERE (f.forum_type=1) '
             . $where1
             . ' AND (f.forum_id=t.forum_id) AND (p.post_id=t.topic_last_post_id) '
             . $where2
             . ' ORDER BY '
             . $order
             . ' DESC';
...
function b_xhnewbbex_new_edit($options)
{
