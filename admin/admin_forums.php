 ...
 * $Id: admin_forums.php,v 1.2 2006/03/28 03:58:41 mikhail Exp $
 ***************************************************************************/
/***************************************************************************
...
if (isset($_POST['save']) && '' != $_POST['save']) {
    if (!$_POST['delete']) {
        ...
        if ($r = $xoopsDB->query($sql)) {
            while (false !== ($row = $xoopsDB->fetchArray($r))) {
...
if (isset($_POST['submit']) && !isset($_POST['save'])) {
    ...
    $desc = htmlspecialchars($myrow['forum_desc'], ENT_QUOTES | ENT_HTML5);
...
<?php
if (1 == $myrow['forum_access']) {
        $access1 = 'SELECTED';
    }

    if (2 == $myrow['forum_access']) {
        $access2 = 'SELECTED';
    }

    if (3 == $myrow['forum_access']) {
        $access3 = 'SELECTED';
    } ?>
</tr>
<tr class='bg1' align='left'>
...
$html_yes = $html_no = $show_icons_panel_yes = $show_icons_panel_no = $show_smilies_panel_yes = $show_smilies_panel_no = $sig_yes = $show_name_yes = $show_name_no = $sig_no = '';
    ...
    if (1 == $myrow['allow_html']) {
        $html_yes = 'checked';
    } else {
        $html_no = 'checked';
    }

    if (1 == $myrow['allow_sig']) {
        $sig_yes = 'checked';
    } else {
        $sig_no = 'checked';
    }
    ...
    if (1 == $myrow['show_name']) {
        $show_name_yes = 'checked';
    } else {
        $show_name_no = 'checked';
    }

    if (1 == $myrow['show_icons_panel']) {
        $show_icons_panel_yes = 'checked';
    } else {
        $show_icons_panel_no = 'checked';
    }

    if (1 == $myrow['show_smilies_panel']) {
        $show_smilies_panel_yes = 'checked';
    } else {
        $show_smilies_panel_no = 'checked';
    }
...
. '<tr><td class="odd">';
    ...
    $cat_title = htmlspecialchars($cat_data['cat_title'], ENT_QUOTES | ENT_HTML5); ?>
<form action="<?php echo $HTTP_SERVER_VARS['PHP_SELF']; ?>" method='post'>
<table border="0" cellpadding="1" cellspacing="0" align='center' Valign="TOP" width="95%"><tr><td class='bg2'>
<table border="0" cellpadding="1" cellspacing="1" width="100%">
<tr class='bg3' align='left'>
...
while ($cat_data = $xoopsDB->fetchArray($result)) {
            echo '<option value="' . $cat_data['cat_id'] . '">' . htmlspecialchars($cat_data['cat_title'], ENT_QUOTES | ENT_HTML5) . "</option>\n";
        } ?>
</select></td>
<tr class='bg3' align='left'>
<td align='center' colspan="2">
...
$sql = 'SELECT * FROM ' . $xoopsDB->prefix('bbex_categories') . ' ORDER BY cat_title';
    ...
    while ($m = $xoopsDB->fetchArray($r)) {
        echo '<option value="' . $m['cat_id'] . '">' . htmlspecialchars($m['cat_title'], ENT_QUOTES | ENT_HTML5) . "</option>\n";
    } ?>
</select>
...
. '<tr><td class="odd">';
    ...
    $show_icons_panel = (int)$_POST['show_icons_panel'];

    $show_smilies_panel = (int)$_POST['show_smilies_panel'];
    ...
    $sql = 'INSERT INTO ' . $xoopsDB->prefix('bbex_forums') . " (forum_id, forum_name, forum_desc, forum_access, cat_id, forum_type, allow_html, allow_sig,posts_per_page,hot_threshold,topics_per_page,show_name,show_icons_panel,show_smilies_panel) VALUES ($nextid, '$name', '$desc', $forum_access, $cat, $type, '$html', '$sig', $ppp, $hot, $tpp, '$show_name', '$show_icons_panel', '$show_smilies_panel')";
    ...
    $notificationHandler->triggerEvents('global', 0, 'new_forum', $tags);
...
<?php
echo "<tr class='bg1' align='left'><td><span class='fg2'>" . _MDEX_A_ALLOWHTML . "</span></td> <td><input type='radio' name='html' value='1'>" . _MDEX_A_YES . "<input type='radio' name='html' value='0' checked>" . _MDEX_A_NO . "</td></tr>
<tr class='bg1' align='left'><td><span class='fg2'>" . _MDEX_A_ALLOWSIGNATURES . "</span></td>
<td><input type='radio' name='sig' value='1' checked>" . _MDEX_A_YES . "<input type='radio' name='sig' value='0'>" . _MDEX_A_NO . '</td></tr>';

    // Ajout Hervé

    echo "<tr class='bg1' align='left'><td><span class='fg2'>" . _MDEX_A_SHOWNAME . "</span></td> <td><input type='radio' name='show_name' value='1'>" . _MDEX_A_YES . "<input type='radio' name='show_name' value='0' checked>" . _MDEX_A_NO . '</td></tr>';

    echo "<tr class='bg1' align='left'><td><span class='fg2'>" . _MDEX_A_SHOWICONSPANEL . "</span></td> <td><input type='radio' name='show_icons_panel' value='1' checked>" . _MDEX_A_YES . "<input type='radio' name='show_icons_panel' value='0'>" . _MDEX_A_NO . '</td></tr>';

    echo "<tr class='bg1' align='left'><td><span class='fg2'>" . _MDEX_A_SHOWSMILIESPANEL . "</span></td> <td><input type='radio' name='show_smilies_panel' value='1' checked>" . _MDEX_A_YES . "<input type='radio' name='show_smilies_panel' value='0'>" . _MDEX_A_NO . '</td></tr>';
...
if (isset($up) && '' != $up) {
    ...
    [$last_number] = $xoopsDB->fetchRow($r);
...
while ($m = $xoopsDB->fetchArray($r)) {
    ...
    echo '<td>' . htmlspecialchars($m['cat_title'], ENT_QUOTES | ENT_HTML5) . "</td>\n";
