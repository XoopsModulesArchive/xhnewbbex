<?php
// $Id: index.php,v 1.4 2006/03/28 03:58:41 mikhail Exp $
// ------------------------------------------------------------------------ //
...
if (isset($mode)) {
    ...
    $LinkToAddCategory = sprintf(
        '<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD VALIGN="middle"><FORM NAME="AddCategory" method="POST" ACTION="%s/modules/xhnewbbex/admin/admin_forums.php" style="margin-bottom:0; padding:0; border:0; margin:0"><input type="hidden" name="mode" value="addcat"><input type="text" name="title" size="40" maxlength="100">&nbsp;<INPUT TYPE="submit" NAME="submit" VALUE="%s"></FORM></TD></TR></TABLE>',
        XOOPS_URL,
        _MDEX_A_CREATENEWCATEGORY
    );
    ...
    if ($result = $xoopsDB->query($sql)) {
        ...
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            ...
            $LinkGo = sprintf('<A HREF="%s/modules/xhnewbbex/index.php?cat=%d">', XOOPS_URL, $categ);

            $LinkEdit = sprintf(
                '<TABLE BORDER=0><TR><TD VALIGN="middle"><FORM NAME="EditCateg" method="POST" ACTION="%s/modules/xhnewbbex/admin/admin_forums.php" style="margin-bottom:0; padding:0; border:0; margin:0"><input type="hidden" name="cat" value="%d"><input type="hidden" name="mode" value="editcat"><INPUT TYPE="submit" NAME="submit" VALUE="%s" ></FORM></TD></TR></TABLE>',
                XOOPS_URL,
                $categ,
                _MDEX_A_EDIT
            );
            ...
            $LinkDelete = sprintf(
                '<TABLE BORDER=0><TR><TD VALIGN="middle"><FORM NAME="DeleteCateg" method="POST" ACTION="%s/modules/xhnewbbex/admin/admin_forums.php" style="margin-bottom:0; padding:0; border:0; margin:0"><input type="hidden" name="cat" value="%d"><input type="hidden" name="mode" value="remcat"><INPUT TYPE="submit" NAME="submit" VALUE="%s" ></FORM></TD></TR></TABLE>',
                XOOPS_URL,
                $categ,
                _MDEX_A_REMOVE
            );
            ...
            if ($result2 = $xoopsDB->query($sql2)) {
                while (false !== ($myrow2 = $xoopsDB->fetchArray($result2))) {
                    ...
                    $LinkGoForum = sprintf('<A HREF="%s/modules/xhnewbbex/viewforum.php?forum=%d">', XOOPS_URL, $forumid);

                    $LinkEdit = sprintf(
                        '<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD VALIGN="middle"><FORM NAME="EditForum" method="POST" ACTION="%s/modules/xhnewbbex/admin/admin_forums.php" style="margin-bottom:0; padding:0; border:0; margin:0"><input type="hidden" name="forum" value="%d"><input type="hidden" name="mode" value="editforum"><INPUT TYPE="submit" NAME="submit" VALUE="%s"></FORM></TD></TR></TABLE>',
                        XOOPS_URL,
                        $forumid,
                        _MDEX_A_EDIT
                    );
                    ...
                    if ('1' == $myrow2['forum_type']) {
                        $LinkPerms = sprintf(
                            '<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD VALIGN="middle"><FORM NAME="PermsForum" method="POST" ACTION="%s/modules/xhnewbbex/admin/admin_priv_forums.php" style="margin-bottom:0; padding:0; border:0; margin:0"><input type="hidden" name="forum" value="%d"><input type="hidden" name="op" value="showform"><INPUT TYPE="submit" NAME="submit" VALUE="%s"></FORM></TD></TR></TABLE>',
                            XOOPS_URL,
                            $forumid,
                            _MDEX_A_EDITPERMS
                        );
                    ...
                    $LinkDelete = sprintf(
                        "<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0><TR><TD VALIGN=\"middle\"><FORM NAME=\"EditForum\" method=\"POST\" ACTION=\"%s/modules/xhnewbbex/admin/admin_forums.php\" style=\"margin-bottom:0; padding:0; border:0; margin:0\"><input type=\"hidden\" name='delete' value=\"1\"><input type=\"hidden\" name=\"forum\" value=\"%d\"><input type=\"hidden\" name=\"mode\" value=\"editforum\"><INPUT TYPE=\"submit\" NAME=\"save\" VALUE=\"%s\"></FORM></TD></TR></TABLE>",
                        XOOPS_URL,
                        $forumid,
                        _MDEX_A_REMOVE
                    );
